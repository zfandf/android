 ...
 * will not work.
 */
public abstract class PictureCursorTreeAdapter extends BaseExpandableListAdapter implements Filterable,
     ...
     */
    protected SparseArray<MyChildCursorHelper> mChildrenCursorHelpers;
     ...
     */
    public PictureCursorTreeAdapter(Cursor cursor, Context context) {
     ...
     *        always displayed.
     */
    public PictureCursorTreeAdapter(Cursor cursor, Context context, boolean autoRequery) {
	...
	private void init(Cursor cursor, Context context, boolean autoRequery) {
        ...
        mChildrenCursorHelpers = new SparseArray<MyChildCursorHelper>();
    }
     ...
     */
    protected synchronized MyChildCursorHelper getChildrenCursorHelper(int groupPosition, boolean requestCursor) {
        MyChildCursorHelper cursorHelper = mChildrenCursorHelpers.get(groupPosition);
        ...
        if (cursorHelper == null) {
            ...
            cursorHelper = new MyChildCursorHelper(cursor);
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild,
        ...
        MyChildCursorHelper cursorHelper = getChildrenCursorHelper(groupPosition, true);
        ...
        bindChildView(v, mContext, cursor, isLastChild, groupPosition);
        return v;
    }
    ...
    public synchronized void releaseCursorHelpers() {
    	Log.d("cursortree", "mChildrenCursorHelpers >>> "+mChildrenCursorHelpers.size());
    ...
    public void notifyDataSetChanged() {
    	Log.d("cursortree", "notifyDataSetChanged");
    ...
    public void notifyDataSetChanged(boolean releaseCursors) {
        Log.d("cursortree", "notifyDataSetChanged >>> " + releaseCursors);
        if (releaseCursors) {
    ...
    public class MyCursorHelper {
        ...
        protected MyContentObserver mContentObserver;
        ...
        protected long getId(int position) {
            if (mDataValid && mCursor != null) {
        ...
        protected Cursor moveTo(int position) {
        ...
        protected void changeCursor(Cursor cursor, boolean releaseCursors) {
        ...
        private class MyContentObserver extends ContentObserver {
            ...
            public void onChange(boolean selfChange) {
                if (mAutoRequery && mCursor != null) {
                    if (false) Log.d("Cursor", "Auto requerying " + mCursor +
                            " due to update");
    ...
    public class MyChildCursorHelper extends MyCursorHelper {
        public MyChildCursorHelper(Cursor cursor) {
        	super(cursor);
        }
        ...
        public long getId(int position) {
            if (mDataValid && mCursor != null) {
                if (mCursor.moveToPosition(4 * position)) { 
                    return mCursor.getLong(mRowIDColumn);
                } else {
                    return 0;
                }
        ...
        public Cursor moveTo(int position) {
            if (mDataValid && (mCursor != null) && mCursor.moveToPosition(4 * position)) {
                return mCursor;
            } else {
                return null;
            }
