...
var List = {
    ...
    fetchList: function(path, hideLoading) {
        if ($('.J_Storage').hasClass('photo')) {
            List.fetchPhotos(path, hideLoading);
        } else {
            List.fetchFile(path, hideLoading);
        }
    },

    fetchFile: function(path, hideLoading) {
        if (!path) {
    ...
    getPhotoCategory: function(action) {
        var oParam = {};
        oParam.data = {
        ...
        oParam.callback = function(oData) {
            if (oData.photos && oData.photos.length > 0) {
                var content = '';
                for (var i = 0; i < oData.photos.length; i++) {
                    var od = oData.photos[i];
                    content += '<option value="' + od.bucket_id +'">' + od.name + '</option>';
                }
                $('.J_Storage').html(content).addClass('photo');
                CrumbNav.initData();
            }
        }
        $.common.ajax(oParam);
    ...
    // 获得图片
    fetchPhotos: function(bucket_id) {
        if (!bucket_id) {
            bucket_id = $('.J_CrumbPath:last').attr('path');
        }
        var oParam = {};
        oParam.data = {
            action: 'photos',
        ...
        oParam.callback = function(oData) {
            ...
            if (oData.photos && oData.photos.length > 0) {
                var str = '';
                for (var i = 0; i < oData.photos.length; i++) {
                    var file = oData.photos[i];
                    file.cls = 'file photo';
                    file.img = '<img src="'+GO.host+'?action=img&is_thumb=1&path='+file.path+'" class="img-thumbnail">';
                    // file.img = '<img src="'+file.path+'" class="img-thumbnail">';
                    str += $.common.setTemplate(List.sPhotoTmpl, file);
                }
                List.eFilesList.html(str);
                Operate.eOperateBtns.find('.btn').addClass('disabled');
                $('#J_SelectAll').removeClass('selected');
...
var Menu = {
    ...
    initEvent: function() {
        ...
        target.on('click', function(e) {
            ...
            var action = $(this).attr('action');
            if (action == 'photos') {
                $('.J_ViewType').addClass('hide');
                $('.J_ViewType[view-type=grid]').click();
                List.getPhotoCategory(action);
            } else if (action == 'info') {
                $('.J_ViewType').removeClass('hide');
                GO.getInfo(action);
            }
            GO.setWindowSize();
        });
        ...
        oParam.callback = function(oData) {
            if (oData.storages) {
                ...
                if (!refresh) {
                    ...
                    $('.J_Storage').html(content).removeClass('photo');
                    CrumbNav.init();
